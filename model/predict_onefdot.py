...
def main(modelpath,svpath,data_root,name,esmfolder,negativefold):
    batch_size = 128
    net = Featuredotmodel(in_dim=1280,hidden_dim=100,out_dim=1280)
    ...
    with torch.no_grad():
        ...
        with ck.progressbar(length=test_steps, show_pos=True) as bar:
            for batch_features,bathch_indexs in pred_loader:
                ...
                newbatch_features = net.predict(batch_features)
                bathch_indexs = torch.squeeze(bathch_indexs)
                # print ('bathch_indexs:\n',bathch_indexs.tolist())
                for i in range(0,len(bathch_indexs)) :
                    ind = bathch_indexs[i]
                    
                    f1 = newbatch_features[i].detach().numpy().tolist()
                    f2 = batch_features[i].detach().numpy().tolist() # esm
                    dd[int(ind)] = [f1,f2]
        print('..sv info',flush=True)

        predallinfo = pd.DataFrame(dd.items(),columns=['index', 'predf1_esm'])
