...
def main(data_root, batch_size, epochs, hiddim, load, device, negativefold,name,esmfolder, modelname,learningrate):
    ...
    if modelname == 'onef':
        ...
        loss_func = nn.BCELoss()
    elif modelname =='generative':
        net = Generatormodel(in_dim=1280,hidden_dim=hiddim,out_dim=1280,number=5)
        loss_func = topnloss
    # loss_func = nn.BCELoss()
    # loss_func = topnloss()
    # lr = 1e-4
            ...
            for epoch in range(epochs):
                ...
                with ck.progressbar(length=train_steps, show_pos=True) as bar:
                    for batch_features, batch_labels in train_loader:
                        ...
                        output = net(batch_features,currdf)
                        ...
                        try:
                            a,b = output
                            loss = loss_func(a,b, batch_labels)

                            # loss = loss_func(output, batch_labels)
                            # loss = loss_func(batch_outputs,batch_pairedvector,batch_labels)
                        except:
                ...
                with torch.no_grad():
                    ...
                    with ck.progressbar(length=valid_steps, show_pos=True) as bar:
                        for batch_features, batch_labels in valid_loader:
                            ...
                            batch_loss = loss_func(a,b, batch_labels)
                            # batch_loss = loss_func(logits, batch_labels)
...
class Generatormodel(nn.Module):
    def __init__(self,in_dim:int,hidden_dim:int, out_dim:int,number:int):
        super().__init__()
        self.__in_dim = in_dim
        self.__out_dim = out_dim
        self.__hidden_dim = hidden_dim
        self.__number = number
        self.linear1 = nn.Linear(self.__in_dim, self.__hidden_dim, bias=False)
        self.linear2 = nn.Linear(self.__hidden_dim, self.__hidden_dim*self.__number, bias=False)
        self.linear3 = nn.Linear(self.__hidden_dim*self.__number, self.__out_dim*self.__number, bias=False)
        self.Sigmoid = nn.Sigmoid()

    def forward(self, x:torch.Tensor,df) -> torch.Tensor:
        # input 1280*2 --> x0[1280] -->[hidden]-->n*[hidden]-->n*[out]
        # loss 
        x0index,x1index = x.chunk(2,axis=1)
        # print('x0index=',x0index[:3])
        ...
        x0 = self.Sigmoid(x0)
        x0 = self.linear3(x0)
        x0 = x0.reshape([batchsize,self.__number,self.__out_dim])
        return x0,x1
...
def topnloss(batch_outputs,batch_pairedvector,batch_labels):
    # print('batch_outputs=\n',batch_outputs)
    ...
    for i in range(0,len(batch_labels)):
        ...
        # out = out.reshape([,int(out.shape[0]/len(batch_labels))])
        # print('size of out={}'.format(out.size()))
        pairv = batch_pairedvector[i]
        currlosslist=[]
        for vv in out:
            # print('vv={}\npairv={}'.format(vv.size(),pairv.size()),flush=True)

            loss = torch.dot(vv.t(),pairv.squeeze())
            currlosslist.append(loss)
        if label == 1:
            # print('label=1|{}'.format(label))
            return min(currlosslist)
        else:
            # print('label=0|{}'.format(label))
            loss =0
            for x in currlosslist:
                loss += len(batch_labels)/x
            return loss



    # torch.cdist(batch_outputs.view(batch_labels.shape[0],number,))
# x0.shape[0],1,x0.shape[1]
